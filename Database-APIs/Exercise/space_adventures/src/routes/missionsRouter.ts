...
missionsRouter.get("/missions", async (req, res) => {
  try {
    ...
    if (req.query["min-landings"]) {
      query.successfulLandings = { $gte: parseInt(req.query["min-landings"] as string, 10) };
    }
...
missionsRouter.get("/missions/find/:id", async (req, res) => {
  try {
    ...
    const mission = await collection.findOne({ _id: new ObjectId(req.params.id) });
    if (!mission) {
...
missionsRouter.put("/missions/:id", async (req, res) => {
  try {
    ...
    if (result.matchedCount === 0) {
      ...
      const updatedMission = await collection.findOne({ _id: new ObjectId(req.params.id) });
